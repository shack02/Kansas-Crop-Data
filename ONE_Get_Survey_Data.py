...
def extract_survey_data():
    csv = "Other Crop Data (Corn, Cotton, Soybeans, Sorghum,  and Wheat).csv"
    df = pd.read_csv(csv)
    print(df)
    df = df[df["Data Item"] == "COTTON, UPLAND - ACRES PLANTED"]
    print(df)
...
def plot_survey_graphs(years_for_each_county, values_for_each_county, counties):
    for county in counties:
        ...
        plt.xticks(x_axis)
        plt.title(county + " Survey: Cotton Acres Planted")
        plt.xlabel("Year")
        plt.ylabel("Value")
